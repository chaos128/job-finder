...
import { AXIS_COLORS } from './axis-colors.js'
...
const badge = cva('inline-flex items-center rounded-full border px-2 py-0.5 text-xs font-medium', {
  variants: {
    variant: {
      ...
      // 다섯 축 고정 색상 — axis-colors.ts 하나에서 가져온다(하드코딩 금지). bg-50/text-700
      // 톤이라 warn(bg-100/text-900)보다 한 단계 옅다 — 경고 배지가 화면을 훑을 때도
      // 이 색들 사이에서 더 진하게 두드러지도록 하는 의도다.
      stack: AXIS_COLORS.stack.chip,
      role: AXIS_COLORS.role.chip,
      domain: AXIS_COLORS.domain.chip,
      growth: AXIS_COLORS.growth.chip,
      conditions: AXIS_COLORS.conditions.chip,
    },
  },
