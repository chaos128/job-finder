...
회사명 매칭은 완전하지 않다(실측 304곳 중 187곳, 62%). `searchCandidates`가
**법인 표기를 먼저 떼고** 원문 → 괄호 앞 → 괄호 안 순으로 시도해 끌어올린 값이다.
미등록으로 확정된 회사도 행으로 남긴다 — 안 그러면 매 실행이 없는 회사를 영원히
다시 조회한다.

**법인 표기 제거가 커버리지를 가장 크게 좌우한다.** 한때 한 글자 `㈜`와 `주식회사`만
떼고 괄호 형태 `(주)`는 그대로 내보냈는데, 그 시점 미등록 174곳 중 82곳(47%)이
`(주)`로 시작했고 `(주)`가 붙은 회사 중 별점이 잡힌 곳은 **0곳**이었다. 떼고 다시
조회하니 87곳 중 57곳이 곧바로 잡혔다(42.8% → 61.5%). 새 표기를 추가할 때는
`LEGAL_FORM` 한 줄만 고치면 된다.

제거 순서가 중요하다 — 법인 표기를 괄호 별칭 분해보다 **먼저** 떼야 한다. 반대로 하면
`노써치(주)`의 괄호 안 `주`가 후보로 올라가 한 글자로 Blind를 뒤진다.
